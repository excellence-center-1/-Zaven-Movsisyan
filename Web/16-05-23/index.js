...
function checkWin(table, turn){
    let rDiag = ((table[0][0] == table[1][1]) && (table[1][1] == table[2][2]) && (table[0][0] != ' '));
    let lDiag = ((table[0][2] == table[1][1]) && (table[1][1] == table[2][0]) && (table[0][2] != ' '));
    let isEmpty = (![table[0].join(''), table[1].join(''), table[2].join('')].join('').includes(' '))
    for(let i = 0; i < table.length; i++){
        let col = ((table[i][0] == table[i][1]) && (table[i][1] == table[i][2]) && (table[i][1] != ' '));
        let row = ((table[0][i] == table[1][i]) && (table[1][i] == table[2][i]) && (table[2][i] != ' '));
        if(col || row){
